...
const NAV = [
  ...
  { id: 'editor',        label: 'Editor',         icon: '✎', countKey: 'chapters' },
  { id: 'characters',    label: 'Characters',     icon: '◉', countKey: 'characters' },
  { id: 'locations',     label: 'Locations',      icon: '◎', countKey: 'locations' },
  { id: 'timeline',      label: 'Timeline',       icon: '◷', countKey: 'events' },
  { id: 'items',         label: 'Items',          icon: '◆', countKey: 'items' },
  { id: 'relationships', label: 'Relationships',  icon: '⬡', countKey: null },
  { id: 'profile',       label: 'Profile',        icon: '👤', countKey: null },
];
...
export default function Sidebar() {
  ...
  const handleSignOut = async () => {
    try { await signOut(); showToast('Signed out', 'info'); }
  ...
  return (
    <aside className="sw-sidebar">
      ...
      <div className="sw-project-pill" onClick={() => setShowProjectPicker(true)} title="Switch project">
        ...
        <div className="sw-project-pill__chevron">▾</div>
      </div>
      ...
      <div className="sw-user-strip">
        {user ? (
          <>
            ...
            <button className="sw-user-strip__signout" onClick={handleSignOut} title="Sign out">↪</button>
          </>
        ) : (
          <button
      ...
      <div className="sw-ai-panel">
        <div className="sw-ai-panel__header">
          <div className="sw-ai-panel__title">
            <span className="sw-ai-panel__icon">✦</span>
            AI Story Analysis
          </div>
        ...
        <div className="sw-ai-panel__message">
          {aiStatus === 'analyzing' && 'Extracting characters, locations, events & items…'}
        ...
        <div className="sw-ai-panel__counts">
          {[
            { icon: '◉', count: (activeProject?.characters || []).length },
            { icon: '◎', count: (activeProject?.locations  || []).length },
            { icon: '◷', count: (activeProject?.events     || []).length },
            { icon: '◆', count: (activeProject?.items      || []).length },
          ].map((t, i) => (
        ...
        <div className="sw-ai-panel__hint">
          Also runs automatically 18s after you stop typing
        </div>
      </div>
    </aside>
  );
